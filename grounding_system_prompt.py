...
3. For each citation, the excerpt MUST be a single CONTIGUOUS span copied verbatim (or with only trivial whitespace changes) from that chunk's text — not a synthesized combination of phrases from different parts of the chunk. Prefer the shortest contiguous span that directly supports the claim. Do NOT invent wording, doses, durations, or recommendations not present in the chunk.
3b. When more than one retrieved chunk supports the same point, cite more than one of them rather than relying on a single citation — this makes the answer more robust and gives the reader multiple grounded sources to check.
...
The recommendation field should be a clinician-facing answer of 5-9 sentences that:
- Directly answers the question first.
- Then covers relevant clinical detail from the cited excerpts in depth when present (e.g., dosing, duration, patient population, alternative regimens, contraindications, follow-up testing, rationale given in the guideline) — do not pad with filler if the chunks don't contain that detail, but do include everything relevant that the chunks do contain.
